...
(function() {
  ...
  //David changed Tuner() to accept one argument
  Tuner = function(coll) {
    ...
    success = function(stream) {
	  ...
	  //David changed the noise threshold from -Infinity to something else
      //make this less than 0.1 but > 0
      ...
      try {
        ...
        process = function() {
          ...
          if (noiseCount < 10) {
			...
			//David changed: fixed typo "Threhold" to "Threshold, but more importantly changing the numbers
			/*
      ...
      getPitch = function(freq) {
		...
		//David changed this to add freq
        return [note, diff, freq];
      };
      display = {
	...
	//David changed; added some functions
	
	//pushes pitch to array that's passed into this Tuner() fn
	//also when it reaches 20 it resets (just like the engrave notes array; sync them eventually!
	function addPitch(coll, pitch) {
		if (pitch >= 0) {
			if (coll.length > NOTEMAX) {
				var n = coll[NOTEMAX-1];
				while (coll[0] != null) {
					coll.pop();
				}
				//console.log(coll);
	...
	//converts note and octave to MIDI number. C4 = 60
	//also engraves onto score
	function noteToMidiNum(note) {
		var conversion = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
		var pitch;
		var octave;
		var pitch_num;
		var engrave_pitch;
		...
		if (note.charAt(2) != "") {
			...
			pitch = note.charAt(0)+note.charAt(1);
			engrave_pitch = note.charAt(0).toLowerCase() + note.charAt(1).toLowerCase() + "/" + octave.toString();
		}
		else {
			octave = note.charAt(1);
			pitch = note.charAt(0);
			engrave_pitch = note.charAt(0).toLowerCase() + "/" + octave.toString();
		}
